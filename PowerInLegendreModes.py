...
from UtilitiesModule import ChoosePlotFile, OverwriteFile
...
class PowersInLegendreModes:
    ...
    def __init__( self, Root, ID, \
        ...
        self.DataDirectory = self.Root + ID + '/'
    ...
    def ComputePowerInLegendreModes( self ):
        ...
        if self.ComputedPowers:
            ...
            Time, P0, P1, P2, P3, P4 = np.loadtxt( self.FileName )

            return Time, P0, P1, P2, P3, P4
...
#        print( 'WARNING: Overwrite set to True. Check near line 274' )
#        Overwrite = True
...
if __name__ == "__main__":
    ...
    P = PowersInLegendreModes( Root, ID, Field, t0 = t0, t1 = t1, \
    ...
    P.PlotPowers( Time[ind], RsAve[ind], RsMin[ind], RsMax[ind], \
                  P0[ind], P1[ind], P2[ind], P3[ind], P4[ind], F )
#
...
#    else:
#
#        fig.suptitle( '{:s}, {:.2f}km'.format( Field, R0 / 1.0e5 ) )
#
#    ind = np.where( ( Time > t0 ) & ( Time < t1 ) )[0]
#
#    Time = Time[ind]
...
#    xlim = ( Time.min(), Time.max() )
#
#    AddPlot( ax, Time, P11, ID1 )
##    AddPlot( ax, Time, P12, ID2 )
#
#    ax.set_yscale( 'log' )
#
#    xlim = ( t0, t1 )
...
#
#        plt.savefig( \
#          'fig.LegendrePowerSpectrum_{:}_{:.2f}-{:.2f}.png'.format \
#          ( Field, fL, fU ), dpi = 300 )
#
#    else:
#
#        plt.savefig( \
#          'fig.LegendrePowerSpectrum_{:}_{:.2f}km.png'.format \
#          ( Field, R0 ), dpi = 300 )
#
#    plt.close()
