...
def MakeMovie2D( ID1, ID2, Field1, Field2, MovieName = 'TwoFields.mp4' ):
    ...
    DataDirectory1 \
      = HOME + 'Research/Data/AccretionShockParameterStudy/{:}/'.format( ID1 )
    DataDirectory2 \
      = HOME + 'Research/Data/AccretionShockParameterStudy/{:}/'.format( ID2 )
    ...
    xL1, xU1, nX1, FileArray1 \
      = MakeDataFile( Field1, DataDirectory1, DataFileName1, PlotFileBaseName1 )

    xL2, xU2, nX2, FileArray2 \
      = MakeDataFile( Field2, DataDirectory2, DataFileName2, PlotFileBaseName2 )
    ...
    for iSS in range( nSS1 ):
        ...
        for iX2 in range( nX1[1] ):
            Data1[iSS,:,iX2] = ( Data1[iSS,:,iX2] - uK1 ) / uK1
            Data2[iSS,:,iX2] = ( Data2[iSS,:,iX2] - uK2 ) / uK2
    ...
    if( zAxisVertical ):
        ...
        time_text = plt.text( 0.125 * np.pi / 2.0, xU1[0] * ( 1.0 + 0.1 ), '' )
    else:
        ax.set_theta_zero_location( 'W' ) # z-axis horizontal
        time_text = plt.text( 0.9 * np.pi / 2.0, xU1[0] * ( 1.0 + 0.3 ), '' )
