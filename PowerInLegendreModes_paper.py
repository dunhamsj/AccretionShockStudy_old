  ...
  def ComputePowerInLegendreModes( self ):
    ...
    if OW:
      ...
      for i in range( nFiles ):
        ...
        if ( i + 1 ) % 10 == 0: print( 'File {:}/{:}'.format( i + 1, nFiles ) )

        ds = yt.load( '{:}'.format( self.DataDirectory + FileArray[i] ) )
        ...
        if self.R0 < 0.0:
          ...
          indX1 = np.where(   ( X1 > self.R0 - dX[0] ) \
                            & ( X1 < self.R0 + dX[0]) )[0]
        ...
        if Field == 'OverPressure':
          ...
          AngleAveragedPressure = np.zeros( (nX[0],nX[1]), np.float64 )

          for j in indX1:
            AngleAveragedPressure[j] = ComputeAngleAverage( AF_P[j,:], X2 )
...
#            -= self.ComputeAngleAverage( Data[j,indX2], X2[indX2] )
        ...
        # --- Decompose data into Legendre modes, i.e.,
        ...
        if self.R0 < 0.0:
          ...
          Power[i,0] \
            = TwoPi \
                * simps( G0[i,indX1]**2 * Psi[indX1]**6 * X1[indX1]**2, \
                         x = X1[indX1] )

          Power[i,1] \
            = TwoPi \
                * simps( G1[i,indX1]**2 * Psi[indX1]**6 * X1[indX1]**2, \
                         x = X1[indX1] )

          Power[i,2] \
            = TwoPi \
                * simps( G2[i,indX1]**2 * Psi[indX1]**6 * X1[indX1]**2, \
                         x = X1[indX1] )

          Power[i,3] \
            = TwoPi \
                * simps( G3[i,indX1]**2 * Psi[indX1]**6 * X1[indX1]**2, \
                         x = X1[indX1] )

          Power[i,4] \
            = TwoPi \
                * simps( G4[i,indX1]**2 * Psi[indX1]**6 * X1[indX1]**2, \
                         x = X1[indX1] )
...
#    F = np.exp( self.FittingFunction( tFit, beta ) )
    ...
    # omega
    self.perr[1] = self.perr[1]
    self.beta[1] = self.beta[1]

    # T ( convert from 1 / ms to ms )
    ### WARNING: MAKE SURE MODS TO PERR HAPPEN BEFORE MODS TO BETA
    self.perr[2] = TwoPi / self.beta[2]**2 * self.perr[2]
    self.beta[2] = TwoPi / self.beta[2]
    #self.perr[2] = self.perr[2] * 1.0e3
    #self.beta[2] = self.beta[2] * 1.0e3
    ...
    if self.Verbose:
      ...
      print( 'omega = {:.3e} Hz'.format( self.beta[1] ) )
      print( 'T     = {:.3e} ms'.format( self.beta[2] ) )
      print( 'dT    = {:.3e} ms'.format( self.perr[2] ) )
  ...
  def PlotData \
    ...
    if type( F ) == np.ndarray:
      ...
      txt = r'$\omega_r$ = ( {:.3e} $\pm$ {:.3f} ) 1/ms'.format \
