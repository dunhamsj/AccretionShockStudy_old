...
Rpns = np.array( [ 2.00e1 ], np.float64 )
    ...
    for m in range( M.shape[0] ):
        for rs in range( Rs.shape[0] ):
            for rpns in range( Rpns.shape[0] ):
                ...
                ID = '{:}1D_M{:}_Mdot0.3_Rs{:}_RPNS2.00e1'.format( R[r], M[m], Rs[rs] )
                ...
                ind = np.where( eta[r,m,rs,rpns] < 0.98 )[0]
...
for m in range( M.shape[0] ):
    for rs in range( Rs.shape[0] ):
        for rpns in range( Rpns.shape[0] ):
            i += 1
            ax[rs].plot( eta[gr,m,rs,rpns], dvdr[gr,m,rs,rpns], \
                         ls = ls[gr], c = color[i], label = R[gr] )
            ax[rs].plot( eta[nr,m,rs,rpns], dvdr[nr,m,rs,rpns], \
                         ls = ls[nr], c = color[i], label = R[nr] )
            ax[rs].text( 0.1, 0.1, r'$\texttt{{M{:}_Rs{:}}}$' \
                                    .format( M[m], Rs[rs] ), \
                         transform = ax[rs].transAxes )
ax[0].legend()
fig.supylabel( r'$dv/dr\ \left[\mathrm{s^{-1}}\right]$')# / ( abs(Vsh) / ( Rsh - Rpns ) )$' )
fig.suptitle( 'Suite 2 (Rpns = 20 km)' )
fig.supxlabel( r'$\eta:=\left(r-R_{\mathrm{S}}\right)/\left(R_{\mathrm{PNS}}-R_{\mathrm{S}}\right)$' )
plt.subplots_adjust( wspace = 0.3 )
#plt.show()
plt.savefig( '/home/kkadoogan/fig.velGrad_suite02.png' )
